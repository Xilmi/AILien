...
public:
	...
	BWAPI::Position         getBaseCenter();
	BWAPI::Position         getEnemyCenter();
	BWAPI::Position			getClosestSunkenPosition();
	BWAPI::Position         getConcaveChoke(BWAPI::Position nearByTo, BWAPI::Position closerTo, BWAPI::Position thanThis);
	...
	std::map<BWAPI::TilePosition, int> getConcaveTileMap(BWAPI::Position concaveCenter, BWAPI::Position concaveEdge, int ringWidth = 0);
	std::vector<BWAPI::Position> getScoutLocations();
};
