...
class ProductionManager
{
	...
	double				_sunkenUnlockTime = 72000;
...
public:
	...
	void        update(double macroHeavyness = 0.15, double sunkensPerWorkerSupply = 0.025, double lingScore = 0, double hydraScore = 0, double lurkerScore = 0, double mutaScore = 0, double ultraScore = 0, double guardScore = 0);
    void        onUnitMorph(BWAPI::Unit unit);
    void        onUnitDestroy(BWAPI::Unit unit);
    void        performBuildOrderSearch();
    void        drawProductionInformation(int x,int y);
    void        setSearchGoal(MetaPairVector & goal);
	void		checkCancels(bool underPressure = false);
	bool		sunkenUnlocked = false;
