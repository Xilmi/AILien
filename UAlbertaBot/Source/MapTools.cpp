...
BWAPI::TilePosition MapTools::getNextExpansion(BWAPI::Player player)
{
    ...
    for (BWTA::BaseLocation * base : BWTA::getBaseLocations())
    {
        ...
        if (!(base == BWTA::getStartLocation(player)))
        {
			...
			{
				if (unit.second.lastPosition.getDistance(base->getPosition()) < 400 && !unit.second.type.isWorker())
				{
					enemyNearby = true;
					break;
				}
			...
			if (base->isMineralOnly()) //Min-Only less viable
			{
				distanceFromHome *= 2;
			}
...
std::vector<BWAPI::Position> MapTools::getScoutLocations()
{
	...
	{
		scoutPositions.push_back(BWAPI::Position(BWAPI::Broodwar->enemy()->getStartLocation()));
	}
	for each(auto base in BWTA::getStartLocations())
	{
		...
		{
			if (element == base->getPosition())
			{
				insert = false;
			}
		}
		if(insert && base->getTilePosition() != BWAPI::Broodwar->self()->getStartLocation())
		{
			scoutPositions.push_back(base->getPosition());
		}
	}
	for each(auto base in BWTA::getBaseLocations())
	{
		bool insert = true;
		for each(auto element in scoutPositions)
		{
...
BWAPI::Position MapTools::getBaseCenter()
{
	...
	for each(auto baseLocation in BWTA::getBaseLocations())
	{
		...
		{
			if (unit->getType().isBuilding() && unit->getDistance(baseLocation->getPosition()) < 200)
			{
				sumX += baseLocation->getPosition().x;
				sumY += baseLocation->getPosition().y;
				spotsWithBuildings++;
				break;
			}
		}
...
BWAPI::Position MapTools::getConcaveChoke(BWAPI::Position nearByTo, BWAPI::Position closerTo, BWAPI::Position thanThis)
{
	...
	{
		if (choke->getCenter().getDistance(closerTo) < choke->getCenter().getDistance(thanThis))
		{
			if (choke->getCenter().getDistance(nearByTo) < closestChokeDistance)
			{
				closestChokeDistance = choke->getCenter().getDistance(nearByTo);
				pos = choke->getCenter();
			}
		}
	}
