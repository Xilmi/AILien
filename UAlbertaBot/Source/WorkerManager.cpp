...
{
	int maxGasWorkers = (workerData.getNumWorkers() / 5.0) * ((BWAPI::Broodwar->self()->minerals() + 8) / (BWAPI::Broodwar->self()->gas() + 8));
...
void WorkerManager::handleCombatWorkers()
{
	for (auto & worker : workerData.getWorkers())
	{
		...
		BWAPI::Unit mineralField = worker->getClosestUnit(BWAPI::Filter::IsMineralField);
		...
		if (target)
		{
			if (!target->isMoving() && worker->getHitPoints() > worker->getType().maxHitPoints() / 2)
			{
				...
				Micro::MutaDanceTarget(worker, target);
			}
			else
			{
				if (mineralField)
	...
	for (auto & unit : BWAPI::Broodwar->self()->getUnits())
	{
		...
		if (unit->getType().isResourceDepot() && (unit->isCompleted() || unit->getType() == BWAPI::UnitTypes::Zerg_Lair || unit->getType() == BWAPI::UnitTypes::Zerg_Hive))
		{
			if (BuildingPlacer::Instance().tileOverlapsBaseLocation(unit->getTilePosition(), BWAPI::UnitTypes::Zerg_Queens_Nest))
			{
				...
				if (workerData.getMineralsNearDepot(unit) + workerData.getGeysersNearDepot(unit) > 0)
				{
					saturation = (double)workerData.getWorkersNearDepot(unit) / (double)workerData.getMineralsNearDepot(unit);
				}
				double enemies = 0;
				if (considerEnemies)
				{
