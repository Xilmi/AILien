...
const bool StrategyManager::needMacroHatch(int hatchCount, double costOfPreferredUnit) const
{
	bool needMacroHatches = false;
	double mineralRate = WorkerManager::Instance().getNumMineralWorkers() * 0.045;
	double gasRate = WorkerManager::Instance().getNumGasWorkers() * 0.07;
	double resourcesPerLarvae = 296 * (mineralRate + gasRate) / hatchCount;
	//BWAPI::Broodwar->drawTextScreen(4, 280, "Income per Larva: %d", int(resourcesPerLarvae));
	if (resourcesPerLarvae > costOfPreferredUnit)
	{
		needMacroHatches = true;
	}
	return needMacroHatches;
}
...
const bool StrategyManager::shouldExpandNow() const
{
	...
	//BWAPI::Broodwar->printf("Mining-bases: %d Saturated: %d",miningBases,saturated);
	return shouldExpand;
}

const bool StrategyManager::baseUnderAttack(bool ignoreWorkers) const
{
	...
	for each(BWAPI::Unit unit in BWAPI::Broodwar->self()->getUnits())
	{
		if (unit->getType() == BWAPI::UnitTypes::Zerg_Hatchery || unit->getType() == BWAPI::UnitTypes::Zerg_Lair || unit->getType() == BWAPI::UnitTypes::Zerg_Hive)
		{
			...
			if (BuildingPlacer::Instance().tileOverlapsBaseLocation(unit->getTilePosition(), BWAPI::UnitTypes::Zerg_Queens_Nest)) //This line makes sure only hatches at gather-locations are counted I must lie to it about what building it is
			{
				...
				if (closestEnemy && closestEnemy->getDistance(unit->getPosition()) < 400)
				{
					if (ignoreWorkers && closestEnemy->getType().isWorker())
					{
						continue;
					}
					else
...
const MetaPairVector StrategyManager::getZergBuildOrderGoal() const
{
	...
	int numUltralisks   = UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Zerg_Ultralisk);
