...
size_t UnitUtil::GetAllUnitCount(BWAPI::UnitType type, bool ignoreUnfinished, double healthToBeConsidered)
{
    size_t count = 0;
    for (const auto & unit : BWAPI::Broodwar->self()->getUnits())
    {
        ...
        if (unit->getType() == type)
        {
			...
			{
				if (healthToBeConsidered > double(unit->getHitPoints()) / double(unit->getType().maxHitPoints()))
				{
					continue;
				}
			}
			count++;
			if (ignoreUnfinished && unit->isBeingConstructed())
			{
				count--;
			}
        }
