...
void UnitData::removeBadUnits()
{
	...
	{
		// If we check where a unit once was and it's not there anymore we should forget about it
		if (BWAPI::Broodwar->isVisible(BWAPI::TilePosition(iter->second.lastPosition)))
		{
			if (!iter->second.unit->isVisible())
			{
				iter->second.positionValid = false;
			}
		}
		if(!iter->second.unit->isVisible())
		{
			iter->second.powerAssigned = 0;
		}
