...
BWAPI::TilePosition BuildingPlacer::getBuildLocationNear(BWAPI::TilePosition pos, const Building & b, int buildDist, bool horizontalOnly) const
{
	// get the precomputed vector of tile positions which are sorted closes to this location
	const std::vector<BWAPI::TilePosition> & closestToBuilding = MapTools::Instance().getClosestTilesTo(BWAPI::Position(pos));
	// iterate through the list until we've found a suitable location
	while (buildDist >= 0)
	{
		for (size_t i(0); i < closestToBuilding.size(); ++i)
...
BWAPI::TilePosition BuildingPlacer::getSunkenPosition()
{
	BWAPI::TilePosition bestSunkenPosition = BWAPI::TilePositions::None;
	BWAPI::Position homePosition = BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation());
	int bestSunkenScore = 0;
	for each(auto baseLocation in BWTA::getBaseLocations())
	{
		...
		for (auto & unit : BWAPI::Broodwar->self()->getUnits())
		{
			...
			if ((unit->getType() == BWAPI::UnitTypes::Zerg_Sunken_Colony || unit->getType() == BWAPI::UnitTypes::Zerg_Creep_Colony) && unit->getPlayer() == BWAPI::Broodwar->self() && unit->getDistance(baseLocation->getPosition()) < 300)
			{
				sunkensAround++;
				currentSunkenPosition = BWAPI::TilePosition(unit->getPosition());
			}
		}
		if (hasHatch)
		{
			sunkenScore = 10 - sunkensAround;
			if (homePosition.getDistance(baseLocation->getPosition()) < 300)
			{
				sunkenScore-=8;
			}
