		...
		for(int x(x0); x<=x1; ++x)
		{
			...
			if(ourUnits)
			{
				for (auto & unit : cell.ourUnits)
				{
					...
					{
						if (d.x * d.x + d.y * d.y <= radiusSq / 4) //Radius for our buildings is halfed as they can't close into the range of the enmy
						{
					...
					else if(d.x * d.x + d.y * d.y <= radiusSq) //Doubled Radius for Our units to accomodate for having a concave
					{
						if (!units.contains(unit)) 
						{
							units.insert(unit);
